...
public partial class EntryViewModel(
    ...
    : AddEditViewModel<Entry, int?>(id, entryService), IAddEditContextAware
{
    ...
    public override async Task InitializeAsync()
    {
        ...
        if (Entity.Relay is not null)
        {
            ...
            OnPropertyChanged(nameof(RelayNumberText));
            // NumberBox binds to RelayNumber, not RelayNumberText — notify so edit loads displayed value.
            OnPropertyChanged(nameof(RelayNumber));
        ...
        if (IsAdd)
        {
            ...
            if (_contextEventId.HasValue)
            {
                var relayContextEvent = RelaySwimEvents.FirstOrDefault(item =>
                    item.Value is SwimEvent se && se.Id == _contextEventId.Value);
                if (relayContextEvent != null)
                {
                    ...
                    if (SelectedRelaySwimEvent?.Value is SwimEvent se && RelayLegs.Count == 0)
                        BuildRelayRowsForCount(se.SwimStyle.RelayCount);
                }
                else
                {
                    SelectedSwimEvent =
                        SwimEvents.FirstOrDefault(item => item.Value is SwimEvent se && se.Id == _contextEventId.Value);
    ...
    private async Task LoadRelayAthletesAsync()
    {
        ...
        foreach (var athlete in athletes)
            RelayAthletes.Add(new SearchableItem { Value = athlete, DisplayText = athlete.DisplayName });
    }
    ...
    private async Task EnsureRelayAthletesContainRelayPositionAthletesAsync()
    {
        ...
        // Prefer already-loaded navigation props (from Include), otherwise fetch by ids.
    ...
    private async Task OnSelectedClubChangedAsync(SearchableItem? value)
    {
        ...
        // Important: do not wipe existing relay positions during initialization (RelayLegs may not be built yet).
    ...
    {
        // When entry creation is scoped to an event context, disallow switching to the wrong entry type.
        ...
        if (value == 1)
        {
            EnsureRelayEntity();
            Entity.AthleteId = null;
            // Only clear declaration time for new relay rows; editing must keep Entry.EntryTime.
            if (IsAdd)
                Entity.EntryTime = null;

            if (SelectedRelaySwimEvent?.Value is SwimEvent swimEvent)
            {
                // Defensive: ensure SwimEventId is set for relay before saving.
                Entity.SwimEventId = swimEvent.Id;
                Entity.SwimStyleId = swimEvent.SwimStyleId;
                BuildRelayRowsForCount(swimEvent.SwimStyle.RelayCount);
            }
    ...
    private async Task TryAssignDefaultRelayNumberAsync()
    {
        ...
        // Determine how many relay entries already exist for this club + relay event.
        ...
        // Requirement: null if only one relay for club, else 2..N.
    ...
    private void BuildRelayLegsFromEntity()
    {
        ...
        // Be defensive: when editing, rely on positions if event/style isn't loaded yet.
        if (relayCount <= 0)
            relayCount = positions.Count;

        if (positions.Count > 0)
        {
            // Ensure we have at least enough legs to show all loaded positions.
    ...
    private void CreateAthlete()
    {
        ...
        if (window is { DialogResult: true, DataContext: IWindowResult { Result: Athlete newAthlete } })
        {
            LoadAthletes();
    ...
    private async Task CreateRelayLegAthleteAsync(RelayRowViewModel? leg)
    {
        ...
        var factory = App.Current.Services.GetRequiredService<IAddEditWindowFactory>();
        var context = new AddEditContext { ClubId = club.Id };
        var window = factory.CreateAndShowAndReturn<AthleteAddEditWindow>(null, context);
    ...
    private void CreateClub()
    {
        ...
        if (window is { DialogResult: true, DataContext: IWindowResult { Result: Club newClub } })
        {
            LoadClubs();
    ...
    public partial class RelayRowViewModel : ObservableObject
    {
        ...
        public string EntryTimeText
        {
            ...
            set
            {
                ...
                if (_entryTimeText != formatted)
                    SetProperty(ref _entryTimeText, formatted);

                if (_entryTime != parsed)
                {
                    _entryTime = parsed;
                    OnPropertyChanged(nameof(EntryTime));
                    _onChanged();
