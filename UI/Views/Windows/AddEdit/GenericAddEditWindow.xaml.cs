...
public partial class GenericAddEditWindow : Window
{
    ...
    public GenericAddEditWindow(object? id, object crudService)
    {
        ...
        if (constructor == null)
            throw new InvalidOperationException(
                $"Could not find constructor for GenericAddEditViewModel<{entityType.Name}, {keyType.Name}> with parameters (TEntity, bool, ICrudService).");
        ...
        Loaded += async (s, e) =>
        {
            var initializeMethod =
                viewModelType.GetMethod("InitializeAsync", BindingFlags.Public | BindingFlags.Instance);
    ...
    private object? LoadEntitySync(object crudService, Type crudServiceType, Type entityType, Type keyType, object id)
    {
        ...
        if (queryMethod == null) queryMethod = crudServiceType.GetMethod("Query", Type.EmptyTypes);
        ...
        var queryable = queryMethod.Invoke(crudService,
            queryMethod.GetParameters().Length > 0 ? new object[] { false } : null);
        if (queryable == null)
