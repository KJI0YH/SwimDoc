...
![Скачивание шаблона в настройках](images/wiki/settings-template-download.png)
...
4. **Очки** рассчитываются автоматически по выбранному в настройках режиму подсчёта (World Aquatics или очки за места).
...
|----------|-------------------------------------------------------------------------------------------|
| **Утвердить заплыв** | Все дорожки заполнены — фиксирует результаты в базе, статус заплыва становится «OFFICIAL», очки события пересчитываются |
...
- **Подсчёт очков** — отдельная страница настроек:
  - режим **World Aquatics** или **командный зачёт**;
  - для WA — редактирование эталонных времён;
  - для командного зачёта — список очков за места; сохранение и пересчёт — кнопкой **Применить**.
  Смена режима **автоматически пересчитывает** очки в открытой базе.
